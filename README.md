...
カード名は画像だけで無理に確定せず、確認済みの `verifiedCardId` がある行だけ掲載数へ登録します。現在確認した76件は73件を登録し、元データの対応カードを確定できない3件は保留しています。確認済み範囲の紐付け率は96.1%で、登録済みカードの誤登録は0件です。検証値は `work/x_buyback_analyzer_metrics.json` に保存します。
