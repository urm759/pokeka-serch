...
出力先には、検出結果の `manifest.json`、カードと価格の切り出し画像、確認用の `contact-sheet.jpg` が作られます。現在のローリエ本舗AKIHABARAの検証画像3枚では、配置と価格を76件中76件正しく検出しています。

カード名は画像だけで無理に確定せず、確認済みの `verifiedCardId` がある行だけ掲載数へ登録します。現在確認した6件は5件を登録し、元データに対応カードがない1件は保留しています。そのため、登録済みカードの誤登録は0件ですが、カード名の自動認識率が76件中76件という意味ではありません。検証値は `work/x_buyback_analyzer_metrics.json` に保存します。
