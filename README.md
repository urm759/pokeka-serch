...
- 平均美品価格は、みんトレ・カードラッシュ・晴れる屋2・遊々亭・トレカキャンプの状態A価格から算出した中央値。特定サイトを常に優先せず、近い価格が過半数を占める場合は多数側を採用します。単独の外れ値は除外し、2店舗の価格が大きく食い違って決められない場合も「異常値・要確認」と表示します
- 「要確認を除外」で、異常値またはデータ不足があるカードを検索結果から省けます
...
最終結論は5種類です。「GO」は現在価格で全条件を満たす、「価格次第」は値下がりだけでGOになる、「要確認」は異常値またはデータ不足がある、「資金不足」は採算条件を満たすが1枚の価格が現在の資金上限を超える、「見送り」は価格を下げても銘柄品質またはリスク条件を満たしません。銘柄品質60点未満は見送り理由へ点数と一緒に表示します。
